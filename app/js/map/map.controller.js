(function() {
  ...
  MapController.$inject = ['$scope', 'isDemoMode', 'routeFetchingService'];

  function MapController($scope, isDemoMode, routeFetchingService) {
    var vm = this;
    vm.map = null;
    ...
    function initBusRoute(map) {
      ...
      routeFetchingService.fetchRoute()
        ...
        function error(response) {
          console.log("Could not load the demo route to draw on the map: ", response.status, response.statusText);
        });
    }
    ...
    if (isDemoMode) {
      $scope.$on('busLocation', function(e, busLocation) {
        var latLng = {
          lat: Number(busLocation.lat),
          lng: Number(busLocation.lng)
        };
